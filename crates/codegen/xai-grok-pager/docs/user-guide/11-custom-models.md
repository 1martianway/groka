...
By default, Grok uses models hosted by SpaceXAI, and new sessions start with `grok-4.6`. Default models require no configuration. Authenticate with `grok login` or an API key, then start a session.
...
```bash
export GROK_WEB_SEARCH_MODEL="grok-4.6"
```
